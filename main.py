...
popen('find ~/PiGro_UE/scripts/ -type f -iname "*.sh" -exec chmod +x {} \;')
...
tab_control.add(tab4, compound=LEFT, text='Tweaks', image=tp04)
...
def contxt_button():
    popen("sudo xdg-open /boot/firmware/config.txt")
...
def z_ram():
    popen("xterm -e 'bash -c \"sudo apt-get install zram-config; exec bash\"'")
...
def inst_lutis():
    popen("sudo chmod +x ~/PiGro_UE/scripts/lutris.sh")
    popen("xterm -e 'bash -c \"sudo ~/PiGro_UE/scripts/lutris.sh; exec bash\"'")
    
def vulk_32():
    popen("xterm -e 'bash -c \"sudo ~/PiGro_UE/scripts/vulkan_32.sh; exec bash\"'")
    
def vulk_64():
    popen("xterm -e 'bash -c \"sudo ~/PiGro_UE/scripts/vulkan_64.sh; exec bash\"'")
    
def tetris_cli():
    popen("xterm -e 'bash -c \"sudo ~/PiGro_UE/scripts/tetris_cli.sh; exec bash\"'")
#####################################TOOLTIPZ
...
#    popen("xterm -e 'bash -c \"~/PiGro_UE/scripts/kuba-yo!.sh && exit; exec bash\"'")
#    tab1.counter += 1
#    L['text'] = str(tab1.counter)


#L = Label(rahmen0, text="", font=("Helvetica", 20), background='#333333', fg="white")
#L.place(y=447, x=315)
...
rahmen0 = Frame(tab1, relief=GROOVE, borderwidth=0)
rahmen0.place(x=150,y=400)
rahmen0['background'] = '#333333'

readfbutton = Button(rahmen0, text="Read First", borderwidth=0, background='green',highlightthickness=1, command=readf,font=(("Helvetica,bold"),"11")).place(x=80,y=400)

#clc_btn0 = Button(rahmen0, image=p9, borderwidth=0, background='white',highlightthickness=0, command=clicked)
#clc_btn0.place(x=120, y=156)
#clc_btn0tt= CreateToolTip(clc_btn0, \
#                                 "You found the hidden button ! Release hell/Kill cute puppies")

Chl = Button(rahmen0, text="Change Log", font="50", width=10, highlightthickness=0, borderwidth=0, background='#333333',
             foreground="white", command=changelog).grid(row=1,column=1)

aclabel=Label(rahmen0,text="JULY FIX of the Fix:",font=("Helvetica", 12), bg="#333333",fg="#d4244d").grid(row=0,column=1)

author = tk.Label(rahmen0, text="Author: Timo Westphal\nVersion: 21.08.19.14:30", foreground="white",font=20,compound=LEFT)
author.grid(row=0,column=0)

paypal = Button(rahmen0, image=pp9, borderwidth=0, background='#333333',highlightthickness=0, command=paypal).place(x=505, y=400)

al = tk.Label(rahmen0, text=r"https://www.actionschnitzel.de/PiGro/", fg="blue", cursor="hand2")
...
def button_action17():
    os.popen("xterm -into %d -bg Grey37 -geometry 120x25 -e ~/PiGro_UE/scripts/addunsignedrepo.sh &" % wid)
...
sys_btn1 = Button(rahmen2, image=bp01, text="Gnome-Tweaks", command=pi_configbutton2,
                  highlightthickness=0, borderwidth=0, background='#333333', foreground="white", compound=TOP, anchor="w")
...
sys_btn3 = Button(rahmen2, image=bp04, text="Gnome-Disks", command=gparted_exec,
                  highlightthickness=0, borderwidth=0, background='#333333', foreground="white", compound=TOP, anchor="w")
...
######################################pop_2000
def ov_2000():
    popen("xterm -e 'bash -c \"~/PiGro_UE/scripts/ov_1.sh && exit; exec bash\"'")
...
######################################pop_2147        
def ov_2147():
    popen("xterm -e 'bash -c \"~/PiGro_UE/scripts/ov_2.sh && exit; exec bash\"'")
...
##########################pop_default
def set_default():
    popen("xterm -e 'bash -c \"sudo chmod +x ~/PiGro_UE/scripts/rm_ov.sh && exit; exec bash\"'")
    popen("xterm -e 'bash -c \"sudo ~/PiGro_UE/scripts/rm_ov.sh && exit; exec bash\"'")
    global pop_default
...
######################################pop_2147        
def ov_2200():
    popen("xterm -e 'bash -c \"~/PiGro_UE/scripts/ov_3.sh && exit; exec bash\"'")
    playsound('scripts/HOLYPiT.mp3')
    global pop_2200
...
def wiki():
    popen("xdg-open https://github.com/actionschnitzel/PiGro_UE/wiki")    
